...
export class RpcClient {
   ...
   * 调用 RPC 方法（HTTP POST）
   */
  private async callHttp<T>(method: string, params?: Record<string, unknown> | unknown[], options: RpcCallOptions = {}): Promise<T> {
    if (options.signal?.aborted)
      throw new RpcError(-32010, 'Request cancelled')
    ...
    try {
      ...
      if (error instanceof Error && error.name === 'AbortError')
        throw new RpcError(-32011, 'Request timeout')
  ...
  private async waitForWebSocketReady(signal?: AbortSignal): Promise<void> {
    if (!signal) {
    ...
    const cancelled = new Promise<never>((_resolve, reject) => {
      cancel = () => reject(new RpcError(-32010, 'Request cancelled'))
      signal.addEventListener('abort', cancel, { once: true })
    })
    try {
      await Promise.race([this.ensureWebSocketReady(), cancelled])
    }
    finally {
      if (cancel)
  ...
  private initWebSocket(): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      connectTimeoutId = setTimeout(() => {
        ...
        settle(() => {
          reject(new RpcError(-32011, `WebSocket connection timeout after ${this.timeout}ms`))
        })
      }, this.timeout)
   ...
   * 调用 RPC 方法（WebSocket）
   */
  private async callWebSocket<T>(method: string, params?: Record<string, unknown> | unknown[], timeoutMs = this.timeout, options: RpcCallOptions = {}): Promise<T> {
    if (options.signal?.aborted)
      throw new RpcError(-32010, 'Request cancelled')

    await this.waitForWebSocketReady(options.signal)
    ...
    return new Promise((resolve, reject) => {
      ...
      const cancel = () => {
        ...
        reject(new RpcError(-32010, 'Request cancelled'))
      }
      const cleanup = () => options.signal?.removeEventListener('abort', cancel)
      timer = setTimeout(() => {
        this.pendingRequests.delete(id)
        cleanup()
        reject(new RpcError(-32011, 'Request timeout'))
      }, timeoutMs)
   ...
   * 调用 RPC 方法
   */
  async call<T>(method: string, params?: Record<string, unknown> | unknown[], options: RpcCallOptions = {}): Promise<T> {
    if (this.useWebSocket) {
      return this.callWebSocket<T>(method, params, this.timeout, options)
    }
  ...
  setTransport(useWebSocket: boolean): void {
    if (this.useWebSocket !== useWebSocket) {
      ...
      if (!useWebSocket && this.ws) {
        this.rejectPendingRequests(new RpcError(-32000, 'WebSocket transport disabled'))
        this.ws.close()
        this.ws = null
      }
    }
...
export class KomariRpc {
  ...
  async getMethods(): Promise<string[]> {
    return this.client.call<string[]>('rpc.methods')
  }
  ...
  async getRecords(params: {
    ...
    load_type?: string
    maxCount?: number
  }): Promise<{ count: number, records: StatusRecord[] | Record<string, StatusRecord[]>, from: string, to: string }> {
   ...
   * 获取负载记录
   */
  async getLoadRecords(uuid?: string, hours?: number, loadType?: string, maxCount?: number): Promise<{ count: number, records: StatusRecord[] | Record<string, StatusRecord[]>, from: string, to: string }> {
    return this.client.call<{ count: number, records: StatusRecord[] | Record<string, StatusRecord[]>, from: string, to: string }>('common:getRecords', {
      type: 'load',
      uuid,
      hours,
      load_type: loadType,
