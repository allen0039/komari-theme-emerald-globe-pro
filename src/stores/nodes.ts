...
const useNodesStore = defineStore('nodes', () => {
   ...
   * 更新节点的状态数据
   */
  function updateNodeStatus(node: NodeData, status: StatusData, statusObserved = true): NodeData {
    return {
      ...node,
      statusObserved,
  ...
  function updateNodeStatuses(statuses: Record<string, NodeStatus>): void {
    ...
    const observedUuids = new Set(Object.keys(statuses))
    ...
    nodes.value.forEach((node, index) => {
      if (!node.statusObserved || observedUuids.has(node.uuid))
        return
      nodes.value[index] = {
        ...node,
        statusObserved: false,
