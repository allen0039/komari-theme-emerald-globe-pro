...
import { DISPLAY_FINANCE_CURRENCIES, isSupportedCurrency } from '@/utils/financeHelper'
import { resolveAnalyticsTimeZone } from '@/utils/history/trafficAggregator'
...
const useAppStore = defineStore('app', () => {
  ...
  const showPublicFinance = computed<boolean>(() => {
    const settings = publicSettings.value?.theme_settings
    return settings?.showPublicFinance === true
  })

  const publicFinanceCurrency = computed(() => {
    const settings = publicSettings.value?.theme_settings
    const configured = typeof settings?.publicFinanceCurrency === 'string'
      ? settings.publicFinanceCurrency.trim().toUpperCase()
      : 'CNY'

    if (
      isSupportedCurrency(configured)
      && (DISPLAY_FINANCE_CURRENCIES as readonly string[]).includes(configured)
    ) {
      return configured
    }
  ...
  const renewalWarningDays = computed<number>(() => {
    const settings = publicSettings.value?.theme_settings
    const configured = settings?.renewalWarningDays
    const value = typeof configured === 'number' || typeof configured === 'string'
      ? Number(configured)
      : Number.NaN
  ...
  const canViewPublicFinance = computed(() => isLoggedIn.value || showPublicFinance.value)

  const analyticsTimeZone = computed<string>(() => {
    const settings = publicSettings.value?.theme_settings
    const configured = settings && typeof settings.analyticsTimeZone === 'string'
      ? settings.analyticsTimeZone.trim()
      : 'browser'
    const candidate = configured || 'browser'

    try {
      resolveAnalyticsTimeZone(candidate)
      return candidate
    }
    catch (error) {
      if (import.meta.env.DEV)
        console.warn(`Invalid analytics time zone \"${candidate}\"; falling back to browser`, error)
      return 'browser'
    }
  })
  ...
  return {
    ...
    offlineNodesLast,
    showPublicFinance,
    publicFinanceCurrency,
    renewalWarningDays,
    canViewPublicFinance,
    analyticsTimeZone,
    icpEnabled,
    icpNumber,
    icpUrl,
