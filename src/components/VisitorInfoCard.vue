...
<template>
  <div class="pointer-events-none fixed inset-x-0 bottom-2.5 z-30 flex justify-center">
    <div
      id="visitor-info-card"
      role="button"
      tabindex="0"
      class="pointer-events-auto max-w-[calc(100vw-1rem)] cursor-pointer overflow-hidden bg-background/30 p-1.5 px-3 text-left shadow-[-1px_-1px_0_background,0_0_16px_rgba(0,0,0,0.05)] outline-none backdrop-blur-sm transition-[border-radius,transform,background-color,box-shadow] duration-300 ease-[cubic-bezier(0.22,1,0.36,1)] hover:bg-background/45 focus-visible:ring-2 focus-visible:ring-emerald-500/50"
      :class="[
        expand
      ...
      aria-controls="visitor-info-details"
    ...
    >
      ...
      >
        ...
        >
          ...
          <div
            class="min-w-0 translate-y-0 opacity-100 transition-[opacity,transform] duration-220 ease-[cubic-bezier(0.22,1,0.36,1)]"
          >
            <div v-if="loading" class="h-2 w-15 animate-pulse rounded-full bg-muted/70" />
            <p
              v-else
              class="max-w-30 text-xs font-medium text-muted-foreground sm:max-w-50"
              :class="expand && item.breakAll ? 'break-all whitespace-normal' : 'truncate'"
            >
              {{ item.value }}
            </p>
          </div>
